...
export default function DashboardClient() {
  ...
  const activeOnly = useMemo(() => employees.filter(e => (e as any).active !== false).sort((a, b) => { const lastA = a.name.split(" ").pop()?.toLowerCase() || ""; const lastB = b.name.split(" ").pop()?.toLowerCase() || ""; return lastA.localeCompare(lastB); }), [employees]);

  const combinedData: CombinedEntry[] = useMemo(() => activeOnly.map(employee => {
    ...
    <div className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
            ...
            <CardContent>
                <div className="text-2xl font-bold">{activeOnly.length}</div>
